...
def read_data():
    ...
    with open("TestAcc_GradClipping.txt", "r") as ins1:
        array_VANILLA = []
        for line in ins1:
            line = line.split()
            temp =float(line[4])
...
def main():
    ...
    plt.figure(1)
    plt.plot(range(no_epochs), data_GRU, label='Normal')
    plt.plot(range(no_epochs), data_2Layers, label='Gradient Clipped',color='Red')
